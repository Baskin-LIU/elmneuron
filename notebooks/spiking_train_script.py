...
from src.LIF_surrogate import LIF, self_inhibit
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inh_tau", type=float, default=4.0)
    parser.add_argument("--inh_strength", type=float, default=0.6)
    ...
    train_config["num_workers"] = 2 if args.machine=="PC" else args.num_workers # will make run nondeterministic
    ...
    for epoch in range(train_config["num_epochs"]):
        ...
        for i, data in pbar:
            ...
            optimizer.zero_grad()
            v, s, inh, v_clamp, elm_out = model(inputs)
            loss = 0.5*BCEloss(s, targets[0]) + 0.5*MSEloss(v_clamp, targets[1])
            loss.backward()
            optimizer.step()
            scheduler.step()
